describe("DraggableDirective", function() {
  ...
  it("should invoke the widgetService with the 'handle' and 'stack' option", function() {
    scope.$eval = jasmine.createSpy("scope.$eval()").and.returnValue({handleSelector: 'test-handle-selector', stackSelector: 'test-stack-selector'});
  ...
  it("should invoke the widgetService with the 'stop' callback emitting the given event", function() {
    ...
    scope.$eval = jasmine.createSpy("scope.$eval()").and.returnValue({onDragStop: "TestEvent"});

    linkFunction(scope, "test-element", {"jbDraggable": "test-map"});

    expect(scope.$eval).toHaveBeenCalledWith("test-map");
    expect(widgetService.makeDraggable).toHaveBeenCalledWith("test-element", {handle: undefined, stack: undefined, stop: jasmine.any(Function)});
