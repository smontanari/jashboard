Feature: Create a new monitor
  ...
  Scenario Outline: Create Monitor returns the persisted monitor with its id
    ...
    When I request the creation of a monitor for dashboard "dashboard_1" with <name>, <refresh_interval>, of type <type>, configured as <config>
  ...
  Scenario Outline: Create Monitor links the monitor to the corresponding dashboard
    ...
    Examples:
    | name         | refresh_interval | type  | config                                                                           |
    | test monitor | 1000             | build | {type: "jenkins", hostname: "test.host.com", port: 8082, build_id: "test_build"} |
